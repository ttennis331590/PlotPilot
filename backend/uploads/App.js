...
function App() {
  ...
  async function uploadChangedFiles() {
    ...
    // Wait for the user to be resolved and set the userName
    ...
    // Read the changed files and append them to formData
    ...
    try {
      const response = await fetch("http://localhost:3001/commit", {
        method: "POST",
        body: formData,
      });
